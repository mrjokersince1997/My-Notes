...
阿里使用 Java 开发的开源消息中间件。

**特点**：纯 JAVA 实现，且集群和 HA 实现都很简单。跟 kafka 相比吞吐率稍低，但是在保持一定的吞吐情况下，在发生宕机和其它故障时消息丢失率更低。因为无论是同步还是异步发送，生产者都会收到实时响应。适合处理高可靠性的数据，被广泛应用在订单，交易，充值，流计算，消息推送，日志流式处理，binglog 分发等场景。
...
为最大化吞吐量，Broker 往往只起到中转和存储的作用，业务逻辑主要在生产者和消费者上进行处理。在顺序消费中提供一个分布式锁，负责保障消费者对 queue 的顺序读取。
...
从逻辑上对消息进行二级分类，消费者需要订阅指定的 topic 甚至 Tag。常用于区分同一个应用下不同的业务或场景。
...
代表一条消息，使用 messageId 唯一识别。 RocketMQ 不对消息的格式做限制，message body 是二进制，序列化操作由用户完成。

用户在发送时可以设置 messageKey ，便于之后查询和跟踪。
